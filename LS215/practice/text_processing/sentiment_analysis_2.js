...
function sentiment(text) {
  const positiveMatches = text.toLowerCase().match(positiveRegex);
  const negativeMatches = text.toLowerCase().match(negativeRegex);
  const resonance       = positiveMatches.length + (-1 * negativeMatches.length);
  let sentiment;
  if (resonance === 0) {
    sentiment = 'Neutral';
  } else {
  ...
  console.log(`There are ${positiveMatches.length} positive type words in the text:`);
  console.log(`Positive sentiments: ${positiveMatches.join(', ')}\n`);
  console.log(`There are ${negativeMatches.length} negative type words in the text:`);
