...
//TODO all that's really changing here are the inputs. Refactor these three into single function.
function onlyWithArraysTester(fName) {
  return function () {
    const inputs = [42, 'howdy!', null, undefined, {toInfinity: 'and beyond!'}, /abc123/];
...
function onlyWithObjectsTester(fName) {
  return function () {
    const inputs = [42, 'howdy!', null, undefined];
    const results = inputs.map((inp) => {
      let rez = null;
      try {
        rez = _(inp)[fName]();
...
function onlyArraysWithObjectsTester(fName) {
  return function () {
    const input = [{a: 1}, 'howdy!', {b: 7}];
    const results = input.map((inp) => {
      let rez = null;
      try {
        rez = _(inp)[fName]();
      } catch (e) {
...
test('first returns null given an empty array', function() {
  return _([]).first() === null;
});
...
test('last returns null given an empty array', function() {
  return _([]).last() === null;
});
...
test('without returns an empty array when given an empty array', function() {
  const rez = _([]).without(7);
  return Array.isArray(rez) && rez.length === 0;
});
test('without returns an empty array when all elements are the chosen em', function() {
  const rez = _([null, null, null]).without(null);
  return Array.isArray(rez) && rez.length === 0;
});
...
test('lastIndexOf works when given empty array', function() {
  return _([]).lastIndexOf(6) === -1;
});
test("lastIndexOf returns last index of supplied value", function() {
  return _([1, 1, 1]).lastIndexOf(1) === 2 && _([1, 2, 3]).lastIndexOf(2) === 1;
});
test("lastIndexOf returns -1 if value is not present", function() {
  return _([1, 1, 1]).lastIndexOf(7) === -1 && _([]).lastIndexOf(2) === -1;
});

test("sample is defined", function() {
  return typeof _().sample === "function";
});
test('sample returns null when input array has no elements', function() {
  return _([]).sample() === null;
});
...
test('sample returns no more than the number of elements in the given array', function() {
  return _([1, 2, 3]).sample(5).length === 3;
});

// test that all array methods only work with arrays!
//   (except sample, which takes ints)
// there are also some object methods taking arrays for input, which are included as well
const arrayMethods = ['first', 'last', 'without', 'lastIndexOf', 'sample', 'pluck', 'findWhere', 'where'];
arrayMethods.forEach((m) => test(`${m} throws error if given anything other than an array`, onlyWithArraysTester(m)));
...
test('pluck returns empty array when no objects have the given key', function() {
  var coll = [{ foo: "bar" }, { foo: "baz" }],
      pluck = _(coll).pluck('gorilla');

  return pluck.length === 0;
});
...
test("pluck ignores objects without requested key", function() {
  var coll = [{ foo: "bar" }, { foop: "baz" }],
      pluck = _(coll).pluck("foo");

  return pluck[0] === "bar" && pluck.length === 1;
});
...
test('keys returns empty array from empty object', function() {
  return _({}).keys().length === 0;
});

test("values is defined", function() {
  return typeof _().values === "function";
});
test('values returns empty array from empty object', function() {
  return _({}).values().length === 0;
});
...
test('extend returns original object', function() {
  const obj = {i: 'love you'};
  return _.extend(obj, {yes: 'i do'}) === obj;
});
test('extend returns original object even without extensions', function() {
  const obj = {i: 'love you'};
  return _.extend(obj) === obj;
});
test('extend returns original object unaltered when given empty object extension', function() {
  const obj = {i: 'love you'};
  const extended = _.extend(obj, {});
  return obj === extended && Object.keys(obj).length === Object.keys(extended).length;
});
test('extend throws error when given non-object', function() {
  const obj = {i: 'love you'};
  try {
    _.extend(obj, 'yes', 'i do', 42);
  } catch(e) {
    return true;
  }
  return false;
...
test('pick returns empty object when given empty object', function() {
  return Object.keys(_({}).pick('anything', 'goes')).length === 0;
});
...
test('omit returns empty object when given empty object', function() {
  return Object.keys(_({}).omit('anything', 'goes')).length === 0;
});
...
test('has can check for multiple properties and only returns true if all are present', function() {
  var o = { foo: 'bar', bar: 'foo' };
  return _(o).has('foo', 'bar') && !(_(o).has('foo', 'bird'));
});
test('has returns false with empty object', function() {
  var o = {};
  return !_(o).has('foo');
});
...
// test that all object methods only work with objects!
// we must ask if Arrays should be considered objects here. eh, why not?
const objectMethods = ['keys', 'values', 'extend', 'pick', 'omit', 'has'];
objectMethods.forEach((m) => test(`${m} throws error if given anything other than an object`, onlyWithObjectsTester(m)));

const comboMethods = ['findWhere', 'where', 'pluck'];
comboMethods.forEach((m) => test(`${m} throws error if its input array contains a non-object`, onlyArraysWithObjectsTester(m)));
